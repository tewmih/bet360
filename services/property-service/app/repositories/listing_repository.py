...
class ListingRepository:
    ...
    async def create(self, listing_data: Dict[str, Any]):
    ...
    async def update(self, listing_id: UUID, update_id: Dict[str, Any]) -> Optional[Listing]:
        """ Update a listing."""
        ...
        #update fields
        for key, value in update_id.items():
    ...
    async def search(
        ...
        if min_price is not None:
            query = query.where(Listing.pricing["amount"].astext.cast(Integer) >= min_price)

        if max_price is not None:
            query = query.where(Listing.pricing["amount"].astext.cast(Integer) <= max_price)

        if bedrooms is not None:
            query = query.where(Listing.property_details["bedrooms"].astext.cast(Integer) == bedrooms)

        if bathrooms is not None:
            query = query.where(Listing.property_details["bathrooms"] == bathrooms)
